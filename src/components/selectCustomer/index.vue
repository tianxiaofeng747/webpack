...
export default {
    ...
    methods: {
        createFilter (queryString) {
            return item =>
                item.label.indexOf(queryString) === 0 ||
                (item.pinyin &&
                    item.pinyin.indexOf(queryString.toUpperCase()) === 0);
        },
        ...
        init () {
            ...
            let list = this.list
                .map(item => {
                    item.pinyin = Pinyin.getCamelChars(
                        item.label
                    ).toUpperCase();
                    return item;
                })
        ...
        selectLetter (key) {
            ...
            el = (el || {}).offsetTop;
            let scrollBar = this.$refs.views.$el.querySelector(
                '.scrollbar-wrapper'
            );
...
.my-customer {
    ...
    .letter {
        width: 20px;
        border-top: 1px solid $menu-right-line;
        li {
            height: 20px;
            text-align: center;
            cursor: pointer;
            font-size: 14px;
            border-bottom: 1px solid $menu-right-line;
            border-left: 1px solid $menu-right-line;
            &:hover {
                color: $theam;
            }
        }
